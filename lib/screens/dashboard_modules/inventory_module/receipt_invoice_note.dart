...
import '../../../models/sales_vcDtl_model.dart';
import '../../../services/api_service.dart';

class InventoryDataController extends GetxController {
  var data = <SalesVcDtlModel>[].obs;

  Future<List<SalesVcDtlModel>> fetchListDetails(String text) async {
    String srno = text;
    // String srno = "2024-01-BPV-0040";
    debugPrint('Voucher Number this time is : $text');
    var url = Uri.parse(
        '${ApiService.baseUrl}GRNGoodsReceiptNote/getListDtl?srno=$srno');
    var response = await http.get(url);

    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      var listData =
          jsonResponse.map((data) => SalesVcDtlModel.fromJson(data)).toList();
      data.value = listData;
      return jsonResponse
...
class ReceiptInvoiceNote extends StatefulWidget {
  const ReceiptInvoiceNote({super.key});

  @override
  State<ReceiptInvoiceNote> createState() => _SalesInvoiceState();
}

class _SalesInvoiceState extends State<ReceiptInvoiceNote>
    with AutomaticKeepAliveClientMixin {
  final InventoryDataController dataController =
      Get.put(InventoryDataController());
  ...
  Future<void> sendPostRequest(String? fkMastValue, String formattedDate,
      String rmks, String usid) async {
    debugPrint("Posted Values : $fkMastValue\n $formattedDate\n $rmks\n $usid");
    try {
      var url =
          Uri.parse('${ApiService.baseUrl}GRNGoodsReceiptNote/PostDGNInvoice');
  ...
  Future<void> submitData() async {
    ...
    //   usid: usid,
    // );
    await sendPostRequest(fkMastValue, formattedDate, rmks, usid);
  }
  ...
  Future deleteRecord(String srno, int trno) async {
    var deleteUrl = Uri.parse(
        '${ApiService.baseUrl}GRNGoodsReceiptNote/DeleteDetailVoucher?srno=$srno&trno=$trno');
  ...
  String? _pkCodeAcc;
  String? _accountValue = " Select Vendor";
  ...
  Future<void> getCustomerAccount() async {
    try {
      final data = await ApiService.get('Balances/PartyBalance');
...
class _MyDialogState extends State<MyDialog> {
  ...
  Future updateDataDialog(String? pkCodeItemList, String? locCodeItemList,
    ...
    if (formKey.currentState!.validate()) {
      ...
      if (discountAmtController.text.isNotEmpty &&
          grossAmtController.text.isNotEmpty) {
      ...
      try {
        var url = Uri.parse(
            '${ApiService.baseUrl}GRNGoodsReceiptNote/UpdateDGNInvoiceDtl');
  ...
  Future submitDataDialog(String? pkCodeItemList, String? locCodeItemList,
    ...
    if (formKey.currentState!.validate()) {
      ...
      try {
        var url = Uri.parse(
            '${ApiService.baseUrl}GRNGoodsReceiptNote/PostDGNnvoiceDtl');
